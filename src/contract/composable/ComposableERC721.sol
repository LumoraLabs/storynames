...
contract ComposableERC721 is ISPGNFT, IComposableERC721, ERC721URIStorageUpgradeable, AccessControlUpgradeable, Ownable {    
    ...
    function rootOwnerOf(uint256 tokenId) public view override returns (address) {
        ...
        uint256 parentTokenId = _tokenIdToOwner[tokenId].parentTokenId;

        while (parentTokenId != 0) {
            tokenId = parentTokenId;
            owner = _tokenIdToOwner[tokenId].ownerAddress;
    ...
    function transferFromParent(address fromContract, uint256 fromTokenId, address to, uint256 tokenId) external override {
